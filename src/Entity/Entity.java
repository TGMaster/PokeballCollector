	...
	public boolean validateNextPosition() {
		...
		if(left) {
			if(colTile == 0 || tileMap.getType(rowTile, colTile - 1) == Tile.BLOCKED) {
                                if (!JukeBox.isPlaying("hitwall")) JukeBox.play("hitwall");
				return false;
			}
                        if(tileMap.getType(rowTile, colTile - 1) == Tile.DAMAGE) {
                                xdest = x - tileSize;
                                Player.isDead = true;
                        }
			else {
		...
		if(up) {
			if(rowTile == 0 || tileMap.getType(rowTile - 1, colTile) == Tile.BLOCKED) {
				...
				return false;
                        ...
                        if(tileMap.getType(rowTile - 1, colTile) == Tile.DAMAGE) {
                                ydest = y - tileSize;
                                Player.isDead = true;
                        }
			else {
